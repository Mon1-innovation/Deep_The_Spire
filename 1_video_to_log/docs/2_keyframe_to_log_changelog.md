...
## 2026-09-26 敌怪、遗物与药水识别

- 为战斗主体和顶部库存新增放大 ROI 复核；战斗页单独复核敌怪名称、血量与意图，所有页面复核遗物和药水栏。
- 将 VLM 识别名称分别匹配到怪物、遗物、药水目录；手牌和奖励/商店选项继续按对应实体类型归一化。模型不再生成实体 ID。
- 日志保留 `raw_name`、`visual_confidence`、目录匹配状态和逐类 `codex_catalogs` provenance；未识别或目录不可用时保留原始名称，不猜测 ID。
- 单类目录加载失败不会阻止其它类别；为实体类型分别缓存目录。
- 观测 schema 升级到 `7`，观测缓存升至 `observation-v8`，旧缓存会自动重跑以触发新增识别调用。
